...
async def get_sp500_tickers() -> List[str]:
    ...
    url = f"{BASE_URL}/sp500-constituent"
    timeout = aiohttp.ClientTimeout(total=None, sock_connect=45, sock_read=45)
    async with aiohttp.ClientSession(headers={"apikey": API_KEY}, timeout=timeout) as s:
        async with s.get(url, timeout=45) as r:
            if r.status != 200:
                raise RuntimeError(f"FMP returned HTTP {r.status} for the constituent list")
            data = await r.json()
...
async def process_ticker(ticker: str, period: str, limit: int, rl: RateLimiter,
    ...
    async with sem:
        try:
            ...
            return {"Ticker": ticker, "_fetch_error": str(e)}
...
async def run(args):
    ...
    timeout = aiohttp.ClientTimeout(total=None, sock_connect=45, sock_read=45)
    async with aiohttp.ClientSession(headers={"apikey": API_KEY}, timeout=timeout) as session:
        tasks = [
    ...
    failure_path = Path(args.out).with_suffix(".failures.txt")
    if "_fetch_error" in df.columns:
        failures = df.index[df["_fetch_error"].notna()].tolist()
    ...
    if failures:
        sys.exit(
            f"❌ {len(failures)} tickers failed. See {failure_path} and rerun; "
