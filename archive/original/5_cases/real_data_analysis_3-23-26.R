...
all_data <- read.csv(
  "sp500_jan_apr_2025_1.17.26.csv",
  header = TRUE
)
event_returns <- read.csv("event_returns.csv")
...
# =========================
# CHOOSE ONE EVENT WINDOW (uncomment!)
# =========================
EVENT_Y <- "S1_Decline"
# EVENT_Y <- "S2_Escalation_Collapse"
# EVENT_Y <- "S3_Policy_Shock_Jump"
# EVENT_Y <- "S4_Uncertainty_Decline"
# EVENT_Y <- "S5_Long_Term_Adjustment"
...
X = data.matrix(all_data[, c(4:13)])   # drop col 3 (Basic.Materials) as baseline
...
sector_cols <- colnames(all_data)[4:13]

mediator_cols <- grep("^(IS|BS|CF)\\.", colnames(all_data), value = TRUE)
M <- data.matrix(scale(all_data[, mediator_cols]))
