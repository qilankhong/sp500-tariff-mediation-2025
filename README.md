...
Stock returns are calculated as close-to-close sums of daily log returns: for a window from `start_date` to `end_date`, the return rows included are `start_date < date <= end_date`. See [Variable definitions](docs/VARIABLES.md) for details on how the dates and mediator variables are defined.
